...
const groenCategories = ['groen-decoratief', 'alle-groen-decoratief', 'fijn-bladgroen', 'grof-bladgroen', 'kerstgroen']
const allProducts = groenCategories.flatMap(slug => getProductsByCategorySlug(slug))
// Remove duplicates by ID
const uniqueProducts = Array.from(new Map(allProducts.map(p => [p.id, p])).values())

const mockFilters = {
  price: [
    { value: '0-20', label: '€0 - €20', count: uniqueProducts.filter(p => parseFloat(p.price) >= 0 && parseFloat(p.price) <= 20).length },
    { value: '20-30', label: '€20 - €30', count: uniqueProducts.filter(p => parseFloat(p.price) > 20 && parseFloat(p.price) <= 30).length },
    { value: '30+', label: '€30+', count: uniqueProducts.filter(p => parseFloat(p.price) > 30).length },
  ],
  availability: [
    { value: 'instock', label: 'Op voorraad', count: uniqueProducts.filter(p => p.stock_status === 'instock').length },
    { value: 'outofstock', label: 'Uitverkocht', count: uniqueProducts.filter(p => p.stock_status === 'outofstock').length },
  ],
}

export default function GroenDecoratiefPage() {
